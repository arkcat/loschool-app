...
const PartyListItem: React.FC<PartyItemProps> = ({ memberId, data }) => {
  ...
  const isMy =
    data.members.filter((char) => {
      return char.member_id === memberId;
    }).length > 0;
  const myChar = data.members
    .filter((char) => {
      return char.member_id === memberId;
    })
    .map((char) => char.char_name);
    ...
    <ListItem>
      <ListItemText
        primary={
          ...
          >
            <span
              style={{ lineHeight: "20px", color: "green" }}
            >{`${data.party.time}:00`}</span>
            <span
              style={{ lineHeight: "20px", paddingLeft: "10px" }}
            >{`${raidInfo.raid_name}`}</span>
            {isMy && (
              <img
                src={questIcon.src}
