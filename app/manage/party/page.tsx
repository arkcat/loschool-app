...
export default function PartyPage() {
  ...
  function getFilteredCharacters(): CharacterData[] {
    ...
    if (currentRaidInfo) {
      const includeRaidCharacters = characterData.filter((character) =>
        currentRaidInfo.raid_group.includes(character.id)
      );
      const canMembers = memberData.filter(
        (member) =>
          member.schedule_check === true && member.schedule[day][time] === 1
      );
      const canCharacters = includeRaidCharacters.filter(
        (character) =>
          canMembers.filter((member) => member.id === character.member_id)
            .length > 0
      );
  ...
  const handleDeleteParty = (id: number) => {
    ...
    if (shouldDelete) {
      setPartyData((prevPartyData) =>
        prevPartyData.filter((party) => party.id !== id)
      );
    }
  };
  ...
  function makePartyBox(partyData: PartyData) {
    ...
    const selectedTimeInt = parseInt(timeSlots[parseInt(selectedTime)]);
    const selected =
      partyData.id === selectedPartyId &&
      partyData.raid_id === selectedRaidId &&
      partyData.day === selectedDayInt &&
      partyData.time === selectedTimeInt;
    return (
      <Box
        key={partyData.id}
        marginBottom={1}
        ...
        >
          ...
          {!isNarrowScreen && (
            <DeleteIcon
              style={{ fontSize: 17 }}
              onClick={() => {
                handleDeleteParty(partyData.id);
              }}
            />
          )}
        </Typography>
        <Typography marginBottom={1} fontSize={14} style={{ color: "white" }}>
          {days[partyData.day]}요일, {partyData.time}시
        </Typography>
        {partyData.member.map((id, characterIdx) => {
          if (id === 0)
            return makeEmptyCharacter(
              String(partyData.id) + String(characterIdx + 900000),
              characterIdx,
              true,
              partyData.id
            );
  ...
  function makeCharacter(
    ...
    return (
      <Card
        ...
        onContextMenu={(e) =>
          handleRightClick(e, showRemove, character, partyId)
        }
      >
        <CardContent style={{ padding: "0 10px" }}>
          <Typography style={{ fontSize: "12px" }}>
            {character.char_name} {character.char_class}{" "}
            {!showRemove && character.char_level}
          </Typography>
        </CardContent>
      </Card>
    );
  ...
  function checkEntryCharacter(characterId: number): boolean {
    ...
    if (raidId === 0 || raidId === 1) {
      filteredParties = partyData.filter(
        (p) => p.raid_id === 0 || p.raid_id === 1
      );
    } else if (raidId === 2 || raidId === 3) {
      filteredParties = partyData.filter(
        (p) => p.raid_id === 2 || p.raid_id === 3
      );
    } else if (raidId === 7 || raidId === 8) {
      filteredParties = partyData.filter(
        (p) => p.raid_id === 7 || p.raid_id === 8
      );
    } else {
      filteredParties = partyData.filter((p) => p.raid_id === raidId);
    }
    const includedParty = filteredParties.filter((p) =>
      p.member.includes(characterId)
    ).length;
  ...
  const getCharacterIDsInParty = useMemo((): number[] => {
    ...
    const ids = partyData
      ...
      .map((id) => {
        return characterData.filter((c) => c.id === id).map((c) => c.member_id);
      })
      .reduce((acc, curr) => {
        curr.forEach((item) => {
          if (!acc.includes(item)) {
  ...
  function handleAddCharacterToParty(e: any, character: CharacterData): void {
    ...
    const updateParty = partyData.map((party) => {
      if (
        ...
        party.raid_id === selectedRaidId &&
        party.day === selectedDayInt &&
        party.time === selectedTimeInt
      ) {
  ...
  function makeCandiCharacter(key: string, character: CharacterData) {
    ...
    const otherCharacterInParty = getCharacterIDsInParty.includes(
      character.member_id
    );
    ...
    return (
      ...
      >
        ...
        <CardContent style={{ padding: "0 10px" }}>
          <Typography style={{ fontSize: "12px" }}>
            {character.char_name} {character.char_class} {character.char_level}
          </Typography>
        </CardContent>
      </Card>
    );
    ...
    return (
      <Box display="flex">
        <Box
          ...
          gap={2}
        >
          <Button variant="contained" onClick={saveParties}>
            파티 편성 확정
          </Button>
        </Box>
        <Box
        ...
        >
          ...
          >
            {raidData?.map((raid, index) => {
              return (
                <MenuItem key={raid.id} value={index}>
                  {raid.raid_name}
                </MenuItem>
              );
            })}
          ...
          <Button variant="contained" onClick={handleAddParty}>
            추가
          </Button>
          <Button variant="contained" onClick={handleDeleteAllParty}>
            전체 삭제
          </Button>
        </Box>
      </Box>
  ...
  const PartyComponent: React.FC<{ party: any }> = ({ party }) => {
    return <Paper style={{ margin: "10px 10px" }}>{makePartyBox(party)}</Paper>;
  };
      ...
      <Grid item xs borderLeft={1}>
        <Grid
          ...
          style={{
            ...
            top: 0,
            zIndex: 1,
            backgroundColor: getDayHeadBgColor(dayData.day),
          }}
        >
          <Typography
            variant="h6"
            ...
            onClick={() => {
              setSelectedDay(String(dayData.index));
            }}
          >
            {dayData.day}
          </Typography>
      ...
      <Box pb={3} pt={8} sx={{ width: "100dvw" }}>
        ...
        <Box display="flex" padding={2} style={{ height: "74dvh" }}>
          ...
          <Box flex={1} border={1} sx={{ overflowY: "auto" }} marginLeft={2}>
            {tableData
              .filter((dayData) => dayData.day === days[parseInt(selectedDay)])
              .map((dayData) => (
                <DayComponent key={dayData.day} dayData={dayData} />
              ))}
          </Box>
  ...
  function makePartyPage() {
    return <Box>{isNarrowScreen ? narrowScreenPage() : wideScreenPage()}</Box>;
  }
