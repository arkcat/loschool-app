...
export default function AttendancePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (memberData) {
          ...
          setColorInfo({
            pColor: memberData.personal_color,
            tColor: memberData.text_color,
          });
        } else {
  ...
  const handleUpdate = async () => {
    try {
      ...
      while (characterNames.length > 0) {
        ...
        console.log(
          `fetch call ${serverCallCount++} with ${characterNames[0]}`
        );
        if (!response) {
  ...
  function checkRelatedRaid(raidId: number, charId: number) {
    return raids
      .filter((raid) => raid.id === raidId)[0]
      .raid_group.includes(charId);
  }
  ...
  function checkRaidDisabled(raid: RaidData, char: CharacterData) {
    ...
    if (raid.id === 40003) {
      ...
      relRaid =
        checkRelatedRaid(40008, char.id) || checkRelatedRaid(40009, char.id);
  ...
  const handleDrop = async (e: any, targetIndex: number) => {
    ...
    const reorderedChars = characters.filter(
    ...
    try {
      await supabase.from("Character").upsert(reorderedItems);
  ...
  function narrowScreenLayout() {
    return (
      <TableContainer
        component={Paper}
        sx={{ maxHeight: "55dvh", width: "85dvw", overflow: "auto" }}
      >
        <Table stickyHeader>
          <TableHead>
            <TableRow>
              ...
              <TableCell
                ...
                style={{ fontFamily: "S-CoreDream-3Light", fontSize: "12px" }}
              >
                레벨
              </TableCell>
              <TableCell
          ...
          <TableBody>
            {characters.map((character: CharacterData, index) => (
              <TableRow key={character.id}>
                <TableCell
                  sx={{ textAlign: "center", borderRight: "2px #f3e07c solid" }}
                  style={{
                    backgroundColor: colorInfo.pColor,
                    color: colorInfo.tColor,
                    position: "sticky",
                    left: 0,
                    zIndex: 98,
                  }}
                  onClick={() => {
                    router.push(
                      `/members/character?id=${getBase64Text(
                        String(character.id)
                      )}`
                    );
                  }}
                ...
                <TableCell sx={{ textAlign: "center" }}>
                  {raids
                    .filter((raid) => raid.raid_group.includes(character.id))
                    .map((raid) => (
                      <Box
                        key={raid.id}
                        borderRadius={2}
                  ...
                  <Box>
                    <Button
                      ...
                      onClick={() => {
                        router.push(
                          `/members/attend-character?id=${getBase64Text(
                            String(character.id)
                          )}`
                        );
                      }}
  ...
  function wideScreenLayout() {
    return (
      <TableContainer
        component={Paper}
        sx={{ maxHeight: "55dvh", maxWidth: "1800px", overflow: "auto" }}
      >
        <Table stickyHeader>
          <TableHead>
            <TableRow>
  ...
  return (
    <MainPageBox>
      ...
      <Box
        ...
        sx={{
          border: "1px #e6bd76 solid",
          backgroundColor: "#f3e07c",
          textAlign: "center",
        }}
        padding="5px"
      >
        <Typography className="page-description">
          캐릭터를 추가하려면 하단 텍스트박스에 캐릭터 이름을 적고 추가 버튼을
          눌러주세요.
          <br />
          업데이트 버튼을 누르면 캐릭터 정보가 서버로부터 업데이트 됩니다.
          <br />
